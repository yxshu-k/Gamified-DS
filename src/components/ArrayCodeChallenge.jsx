...
import codeChallenges from "../data/arraycodechallenge.json";
...
export default function ArrayCodeChallenge() {
  const [index, setIndex] = useState(() =>
    getRandomIndex(codeChallenges?.length || 0)
  );
  ...
  const challenge = codeChallenges && codeChallenges.length > 0 ? codeChallenges[index] : null;
  ...
  const nextChallenge = () => {
    if (!codeChallenges || codeChallenges.length === 0) return;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black text-white flex flex-col items-center justify-center px-4 py-10">
        <div className="text-center">
          ...
          <p className="text-gray-400 mt-2">Please add challenges to arraycodechallenge.json</p>
        </div>
      </div>
    );
  }
